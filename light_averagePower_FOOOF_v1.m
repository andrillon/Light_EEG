...
mdla_1=fitlme(table_alpha,'Freq~1+BlockN+(1|SubID)');
...
mdlb_1=fitlme(table_alpha,'Amp~1+BlockN+(1|SubID)');
...
for nBl=1:5
    for nCond=1:2
    temp1=table_alpha.Freq(table_alpha.BlockN==nBl & table_alpha.Cond==Conds{nCond});
    temp1b=table_alpha.SubID(table_alpha.BlockN==nBl & table_alpha.Cond==Conds{nCond}); temp1b=removecats(temp1b);
    mean1=grpstats(temp1,temp1b);
    simpleBarPlot(nBl+(2*nCond-3)*0.2,mean1,Colors{nCond}(nBl,:),0.35,'k',[],3);
    end
end
...
for nBl=1:5
for nCond=1:2
    temp1=table_alpha.Amp(table_alpha.BlockN==nBl & table_alpha.Cond==Conds{nCond});
    temp1b=table_alpha.SubID(table_alpha.BlockN==nBl & table_alpha.Cond==Conds{nCond}); temp1b=removecats(temp1b);
    mean1=grpstats(temp1,temp1b);
    simpleBarPlot(nBl+(2*nCond-3)*0.2,mean1,Colors{nCond}(nBl,:),0.35,'k',[],3);
end
end
title('Alpha Peak Ampl')
format_fig;
xlabel('Block')
ylabel('Amp')
%%
...
for nCond=1:3
    for nB=1:5
        ...
        for nEl=1:32
            if nCond==3
                temp_topo(nEl)=nanmean(table_alpha.Amp(table_alpha.Cond==Conds{1} & table_alpha.BlockN==nB & table_alpha.ElecN==nEl))-...
                    nanmean(table_alpha.Amp(table_alpha.Cond==Conds{2} & table_alpha.BlockN==nB & table_alpha.ElecN==nEl));
            else
...
for nCond=1:3
    for nB=1:5
        ...
        for nEl=1:32
            if nCond==3
                temp_topo(nEl)=nanmean(table_alpha.Freq(table_alpha.Cond==Conds{1} & table_alpha.BlockN==nB & table_alpha.ElecN==nEl))-...
                    nanmean(table_alpha.Freq(table_alpha.Cond==Conds{2} & table_alpha.BlockN==nB & table_alpha.ElecN==nEl));
            else
                temp_topo(nEl)=nanmean(table_alpha.Amp(table_alpha.Cond==Conds{nCond} & table_alpha.BlockN==nB & table_alpha.ElecN==nEl));
            end
        end
